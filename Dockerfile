...
RUN apt update && \
    ...
    echo "ros2 run vision_perceiver_ros2 vision_perceiver_ros2" >> /home/shade/shade_ws/start.sh && \
    chmod +x ./start.sh

COPY . ./src/vision_perceiver_ros2

RUN pip3 install ./src/vision_perceiver_ros2 && \
    : "Install the model" && \
